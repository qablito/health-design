...
|---|---|---|---|---|
| Invitación y acceso | enlace/código/QR desde Internet | Internet → API | enumeración, replay, expiración | planificado; `docs/operations/OPERATIONS.md` |
...
| Funciones superadmin | panel/endpoint privado | usuario → privilegio | impersonación, borrado, publicación | planificado |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| TM-001 | remoto | endpoint de acceso observable | enumera alias/códigos | exposición de perfiles | sesiones, salud | planificado: alias visible separado del secreto | no implementado | respuesta uniforme, rate limit, código de alta entropía, alertas | intentos por alias/IP y ratio fallo/éxito | alta | crítica | critical |
| TM-002 | invitado | QR capturado | reutiliza QR consumido | sesión no autorizada | sesiones, planes | planificado: un uso y caducidad | no implementado | nonce aleatorio, consumo atómico, TTL corto, invalidación inmediata | replay y consumo duplicado | media | alta | high |
| TM-003 | invitado | sesión propia | cambia IDs de perfil | lectura/escritura cruzada | salud, planes | planificado: RLS y authZ por recurso | no implementado | política RLS por membresía, comprobar rol en servidor, pruebas IDOR | auditoría de denegaciones | media | crítica | critical |
| TM-004 | remoto | token robado mediante dispositivo o XSS | reutiliza sesión | acceso persistente | sesiones, todo el perfil | planificado: actor Auth por dispositivo, membresía y revocación RLS | no implementado | CSP/headers exactos, JWT 15 min, refresh rotation/reuse 10 s, `DeviceSession` 30/180 propia, revocación de perfil separada de cierre global y cleanup huérfano | replay, sesiones vencidas y simultáneas anómalas | media | crítica | high |
| TM-005 | atacante administrativo | cuenta superadmin comprometida | impersona y cambia datos | compromiso total | todos los perfiles y reglas | implementado localmente: AAL2, desafío TOTP no anterior a cinco minutos para iniciar, indicador persistente, actor original/efectivo y ledger cifrado | activación remota pendiente | MFA obligatorio, reautenticación, actor original/efectivo y alertas | log de impersonación, intents pendientes y cambios masivos | media | crítica | critical |
